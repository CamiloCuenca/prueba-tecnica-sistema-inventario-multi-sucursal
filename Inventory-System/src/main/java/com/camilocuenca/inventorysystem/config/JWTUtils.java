...
public class JWTUtils {
    ...
    public String generateToken(String email, Map<String, Object> claims) {
        ...
        return Jwts.builder()
                .setClaims(claims)  // Agregar las claims adicionales al token
                .setSubject(email)  // Establecer el email como el sujeto del token
                .setIssuedAt(Date.from(now))  // Establecer la fecha y hora de emisión del token
                .setExpiration(Date.from(now.plus(1L, ChronoUnit.HOURS)))  // Expira en 1 hora
                .signWith(getKey(), SignatureAlgorithm.HS256)  // Firmar el token utilizando la clave secreta generada y algoritmo HS256
    ...
    public Jws<Claims> parseJwt(String jwtString) throws ExpiredJwtException, UnsupportedJwtException, MalformedJwtException, IllegalArgumentException {
        ...
        // (Opcional) validar explícitamente el algoritmo usado en la cabecera
        String alg = jws.getHeader().getAlgorithm();
        if (!SignatureAlgorithm.HS256.getValue().equals(alg)) {
            throw new UnsupportedJwtException("Algoritmo de firma no soportado: " + alg);
        }
    ...
    private SecretKey getKey() {
        ...
        // Convierte la cadena de texto en un array de bytes, usando UTF-8.
        byte[] secretKeyBytes = claveSecreta.getBytes(StandardCharsets.UTF_8);
